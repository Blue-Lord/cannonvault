...
def vaultScreen():
    ...
    def updateEntry(input):
        ...
        text4= "Username"
        text5= "password"
        username = encrypt(popUp(text4).encode(), encryptionKey)
        password = encrypt(popUp(text5).encode(), encryptionKey)

        
        #update_fields = "UPDATE vault SET (username)= (?), (password)=(?) WHERE id=(website)" " "
        cursor.execute("UPDATE vault SET (username)= (?), (password)=(?) WHERE id=(?)", (input,username,password))
        db.commit()
        vaultScreen()
        
    '''
        update_fields = """UPDATE vault SET (username, password) 
        VALUES( ?, ?)  """
        cursor.execute(update_fields, ( username, password))
        db.commit()
    ...
    if (cursor.fetchall() != None):
        ...
        while True:
            ...
            btn = Button(window, text="Edit", command=  partial(updateEntry))#array[i][0]))
...
#hello69
#sd1df770dfc834894b043f4b184859c87